...
public class SceneFactory {
	private Transfo currTransfo = new Transfo();
	...
	private Shape createShape(String token, Scanner scanner, Material currMtrl){
		...
		shape.setTransfo(new Transfo(this.currTransfo));
		return shape;
	}
	...
	public boolean processTransfo(String token, Scanner scanner){
		...
		if(token.equals(Token.SCALE.toString())){
			...
			this.currTransfo = new ScaleTransfo(sx,sy,sz); 
		}
			...
			this.currTransfo = new TranslateTransfo(tx,ty,tz); 
		}
			...
			this.currTransfo = new RotateTransfo(angle,x,y,z); 
		}
		else{
			isTransfo = false; 
		}
