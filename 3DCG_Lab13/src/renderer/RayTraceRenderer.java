	...
	public void render(GLAutoDrawable drawable) {
		...
		for(int r = 0; r<this.nRows; r++){
			for(int c = 0; c<this.nCols; c++){
				...
				float heightPixel = camera.height/ nRows; 
				
				float deltaW = widthPixel / nSamples; 
				float deltaH = heightPixel / nSamples; 
				
				//Iterate over columns of one pixel
				for(int i = 0; i<nSamples; i++)
				{
					//Iterate over rows of one pixel
					for(int j = 0; j <nSamples; j++){
						
						//Calculate v2 = the vector from the top-left corner to the (top-left-corner of) pixel
						float x = c*widthPixel+i*deltaW;
						float y = r*heightPixel+j*deltaH;
				...
				for(Colour sample : samples){
					colAvg.add(sample.mult(1.0f / (nSamples*nSamples))); 
